...
class SchemaAgent:
            ...
            try:
                ...
                for table_name, data in schema_info["sample_data"].items():
                    for row in data:
                        ...
                        command = f"INSERT OR REPLACE INTO {table_name} ({columns}) VALUES ({values});"
                        self.session.execute(text(command))
                self.session.commit()
    ...
    def save_schema(self, name, schema):
        new_schema = Schema(name=name, schema=schema)
...
class QuestionGenerationAgent:
    ...
    def generate_sql_prompt(self, schema, sample_data, difficulty, statements):
        ...
        # Initialize the instruction based on the selected statements
        if "WHERE" in statements:
            if difficulty == "Level 1" or difficulty == "Level 2":
                complexity_instruction = "Generate a basic SQL question using SELECT, FROM, and WHERE clauses with simple conditions."
            elif difficulty == "Level 3" or difficulty == "Level 4":
...
class ValidationAgent:
    def __init__(self):
        self.model = "gpt-4"  # Model used for validation (this could be the same or different from the generation model)
    ...
    def validate_sql(self, sql, allowed_statements):
        ...
        # If local validation passes, proceed to external API validation
    ...
    def validate_with_api(self, sql):
        ...
        try:
            response = openai.ChatCompletion.create(
                ...
                max_tokens=50  # Keep this small as you only need a "Valid" or "Invalid" response
            )
    ...
    def saved_schemas_page(self):
        ...
        if st.button("Save Schema"):
            if new_schema_name and new_schema_content:
                self.schema_agent.save_schema(new_schema_name, new_schema_content)
...
schemas = {
    "Movie Database": {
        "schema": "CREATE TABLE IF NOT EXISTS movies (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, director TEXT, genre TEXT, release_year INTEGER);",
        "sample_data": {
            "movies": [
    ...
    "School System": {
        "schema": "CREATE TABLE IF NOT EXISTS students (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, grade INTEGER, age INTEGER);\n"
                  "CREATE TABLE IF NOT EXISTS courses (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, instructor TEXT);",
        "sample_data": {
            "students": [
...
schema_agent.initialize_database(schemas)
